...
branch: each SFD shrink fails with probability at most $\delta$, so over
$T$ shrinks Adaptive is a Las Vegas algorithm with cumulative failure
probability $\le n_{\mathrm{sfd}}\,\delta$ via union bound. We do not
formalise the composition --- \eqref{eq:adaptive-bound} is the worst-case
envelope, and the empirical covariance error in
...
Equation~\eqref{eq:rho-eff-eq} accounts for one factor of two in the gap
between the textbook crossover $\rho^\star = 1 - \ell/d \approx 0.96$ and
the empirical crossover near $\rho \approx 0.3$ (Section~\ref{ssec:exp1}).
The textbook derivation equates SFD's $O(\mathrm{nnz}(M)\,\ell)$ leading
term with FD's $O((\ell+r)\,d\,\ell)$, treating
$\mathrm{nnz}(M)/((\ell+r)d)$ as the input density $\rho$; by
\eqref{eq:rho-eff-eq} that ratio is in fact $\rho_{\mathrm{eff}} \approx
2\rho$, so the equation should be balanced at $2\rho^\star$, halving the
predicted threshold to $\approx 0.48$. The remaining gap to the observed
$\approx 0.3$ is absorbed by the $n_{\mathrm{iter}}$ factor on the leading
SFD term and the $(\ell + r)\,\ell^2$ lower-order correction --- both of
which the Adaptive cost model in Algorithm~\ref{alg:adaptive} accounts for
directly because it computes $\mathrm{nnz}(M_{\mathrm{sfd}})$ from the
running buffer rather than from a textbook closed form. This is also the
reason the calibrated $\beta_{\mathrm{sfd}}$ comes out close to
$\alpha_{\mathrm{fd}}$ on the M4 in Section~\ref{ssec:calib}: the cost
model is already paying for $\rho_{\mathrm{eff}}$ inside
$\mathrm{nnz}(M_{\mathrm{sfd}})$, so the residual fit only has to capture
the per-flop hardware cost.
