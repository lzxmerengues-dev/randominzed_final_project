...
recaps FD and SFD in self-contained form. Section~\ref{sec:rho-eff} introduces
\emph{sketch-drift effective density}, a structural observation that the
buffer $M$ becomes dense after a single shrink and therefore
$\rho_{\mathrm{eff}}$ is bounded below by $\ell/(\ell + r_t)$ regardless of
$\rho$. Section~\ref{sec:adaptive} describes Adaptive FD/SFD, an online
selector backed by a hardware-calibrated cost model that chooses the cheaper
subroutine per shrink. Section~\ref{sec:exp} validates both contributions
empirically across synthetic, real, and CPU/GPU settings; Section~\ref{sec:disc}
concludes with limitations and extensions. All claims are validated by direct
measurement; we do not pursue formal proofs.
...
Liberty's FD~\cite{liberty2013fd} maintains a buffer $B \in \mathbb{R}^{2\ell \times d}$.
Incoming rows of $A$ are placed in empty rows of $B$ until the buffer fills;
once full, the algorithm computes a thin SVD $B = U \Sigma V^\top$ and shrinks:
\[
  B \;\leftarrow\; \bigl(\Sigma^2 - \sigma_{\ell+1}^2 I\bigr)_{+}^{1/2} V^\top,
\]
keeping the top $\ell$ rows and zeroing the bottom $\ell$. The shrink subtracts
the $(\ell{+}1)$-th squared singular value from every retained singular value,
ensuring rank $\le \ell$ while conserving the leading directional structure.
After processing the full stream the residual sketch satisfies
\begin{equation}
\label{eq:fd-bound}
  \|A^\top A - B^\top B\|_2 \;\le\; \frac{\|A - A_k\|_F^2}{\ell - k}
  \qquad\text{for any } k < \ell.
\end{equation}
The dominant cost is the $(2\ell \times d)$ SVD repeated $n / \ell$ times,
totaling $O(nd\ell)$ work. FD is density-oblivious: a sparse input is densified
the moment it enters the buffer.

\subsection{Sparse Frequent Directions}
\label{ssec:sfd}

Ghashami, Liberty, and Phillips~\cite{ghashami2016sfd} modify FD to exploit
sparse inputs without paying for materialization. Their three changes are:

\textbf{(a) nnz-based batching.} Rather than filling the buffer up to $2\ell$
rows, accumulate sparse rows of $A$ until total nonzeros reach roughly
$\ell d$. The implicit buffer $M = [B; \mathrm{batch}] \in \mathbb{R}^{(\ell + r) \times d}$
has an asymptotically dense first block (the running sketch $B$) and a sparse
second block (the freshly-arrived rows).

\textbf{(b) Implicit simultaneous iteration.} Replace the exact SVD on $M$
with a randomized rank-$\ell$ approximation in the spirit of Halko, Martinsson,
and Tropp~\cite{halko2011randomsvd}. Draw $\Omega \in \mathbb{R}^{d \times \ell}$,
form $Y_0 = M \Omega$, and run $n_{\mathrm{iter}} \approx 4$ power steps of
$Y_{i+1} = M(M^\top Y_i)$ with QR re-orthogonalization between iterations.
Crucially, every product factors as $M v = [B v;\ \mathrm{batch}\cdot v]$,
so the second term costs only $O(\mathrm{nnz}(\mathrm{batch}))$ flops.
Project $M$ onto the resulting range to obtain a small $(\ell \times d)$
matrix and SVD that exactly. The shrink rule is identical to FD's.

\textbf{(c) Verify and retry.} Power iteration is randomized; the resulting
sketch may violate the FD invariant on a bad $\Omega$ draw. SFD therefore runs
a randomized spectral test that estimates
$\|M^\top M - B_{\mathrm{cand}}^\top B_{\mathrm{cand}}\|_2$ via block power
iteration on the difference, retrying simultaneous iteration if the estimate
exceeds a tolerance $2 \sigma_\ell^2$. The retry budget is
$O(\log(1/\delta))$ in expectation.

The resulting algorithm satisfies, with probability $\ge 1 - \delta$ over its
internal randomness,
\begin{equation}
\label{eq:sfd-bound}
  ...
  \qquad \alpha = 6/41.
\end{equation}
The per-shrink cost is
$O(\mathrm{nnz}(M) \cdot \ell \cdot n_{\mathrm{iter}} + (\ell+r)\ell^2)$.
Comparing this to FD's $(\ell+r)d\ell$ gives the textbook crossover
$\rho \le 1 - \ell/d$, modulo the $n_{\mathrm{iter}}$ constant. For our default
parameters this yields $\rho^\star = 0.96$.
...
(MovieLens, $\ell = 10$) is a regression on a pre-fix variant of Adaptive
discussed in Section~\ref{sec:disc}. The covariance error of Adaptive is
...
\textbf{Reproducibility caveat.} The MovieLens and GPU runs were measured
against a slightly older Adaptive variant before we replaced the per-batch
cost comparison with a per-row one (Section~\ref{ssec:adaptive-algo}). The
synthetic results in Sections~\ref{ssec:exp1}--\ref{ssec:exp3} use the
current implementation; absolute Adaptive numbers in Sections~\ref{ssec:exp4}--\ref{ssec:cpu-gpu}
may improve by 5--15\% with the current code. Trends are consistent across
both variants. All raw JSON results and figures are versioned in our GitHub
repository, and Experiment 5 reproduces on a free Colab account in under
five minutes.
